    ...
    <div className="mess-committee-meeting-container">
      ...
      {userRole === 'warden' && (
        <div className="schedule-meeting-section" style={{ marginBottom: '30px', padding: '20px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <h3 style={{ marginTop: 0, color: '#2b3674' }}>{isEditing ? 'Reschedule Meeting' : 'Schedule New Meeting'}</h3>
        ...
        {meetings.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {meetings.map(meeting => {
              ...
              return (
                <div key={meeting.id} className="meeting-card" style={cardStyle}>
                  ...
                  {userRole === 'warden' && (
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button onClick={() => handleCancel(meeting.id)} style={{ ...actionBtnStyle, color: '#e53e3e', borderColor: '#e53e3e' }}>Cancel</button>
                    </div>
                  )}
                </div>
              );
